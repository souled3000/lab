 ...
 * <ol>
 * <li>{@link KafkaBolt} that receives random sentences from {@link RandomSentenceSpout} and publishes the sentences to a kafka "test" topic.</li>
 * <li>{@link TransactionalTridentKafkaSpout} that consumes sentences from the "test" topic, splits it into words, aggregates and stores the word count in a {@link MemoryMapState}.</li>
 * <li>DRPC query that returns the word counts by querying the trident state (MemoryMapState).</li>
 * </ol>
 * <p>
 * For more background read the <a href="https://storm.apache.org/documentation/Trident-tutorial.html">trident tutorial</a>, <a href="https://storm.apache.org/documentation/Trident-state">trident state</a> and
...
public class TridentKafkaWordCount {
	 ...
	 * <p/>
	 * For more info on transactional spouts see "Transactional spouts" section in <a href="https://storm.apache.org/documentation/Trident-state"> Trident state</a> doc.
	 *
	...
	/**
	 * Creates a trident topology that consumes sentences from the kafka "test" topic using a {@link TransactionalTridentKafkaSpout} computes the word count and stores it in a {@link MemoryMapState}. A DRPC stream is then created to query
	 * the word counts.
	 * 
	 * @param drpc
	 * @return
...
//		conf.setDebug(true);
	...
	/**
	 * A topology that produces random sentences using {@link RandomSentenceSpout} and publishes the sentences using a KafkaBolt to kafka "test" topic.
	 *
	...
	public StormTopology buildProducerTopology(Properties prop) {
		...
		/**
		 * The output field of the RandomSentenceSpout ("word") is provided as the boltMessageField so that this gets written out as the message in the kafka topic.
		 */
	...
	/**
	 * Returns the storm config for the topology that publishes sentences to kafka "test" topic using a kafka bolt. The KAFKA_BROKER_PROPERTIES is needed for the KafkaBolt.
	 *
	 * @return the topology config
	 */
	 ...
	 * </pre>
	 * 
	 * This will submit two topologies, one for the producer and another for the consumer. You can query the results (word counts) by running an external drpc query against the drpc server.
	 */
